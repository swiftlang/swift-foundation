...
private struct HebrewCalendarTests {
    ...
    // DST policy parity tests moved to
    // Tests/FoundationInternationalizationTests/HebrewDSTPolicyParityTests.swift —
    // they need `TimeZone(identifier: "America/Los_Angeles")`, which requires
    // the ICU-backed TimeZone only linked in the Internationalization target.
}
