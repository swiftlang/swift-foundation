...
Pseudoterminal (PTY) support has been a common request during the public beta period. We propose two additional `run()` overloads and their companion types that specifically spawn the child process in PTY mode:
...
public struct Pseudoterminal: Sendable {
    ...
    /// The dimensions of this terminal window
    public let windowSize: WindowSize
    /// The type of this terminal as defined in `terminfo`
    public let terminalType: String

    /// Update the dimensions of this terminal window
...
Both `run()` overloads allow developers to use a custom closure to:

- Stream the terminal output (a combined standard output and standard error stream from the child process);
- Write to the terminal input;
- Control the process running state (send signals);
- Get and update terminal information.
