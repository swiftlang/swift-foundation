...
- A `Configuration` value is independently useful for developers who want to describe a calendar without instantiating one, or use the configuration as a key, similar to how Foundation uses a `Date.FormatStyle` value as a key into a cache of date formatters.
