...
extension Calendar {
    ...
    /// Normalized weeks of year so that negative expand to a week in the end of
    /// the year
    fileprivate func normalizedWeeksOfYear(_ weeksOfYear: [Int], anchor: Date) -> [Int] {
    ...
    fileprivate func _matchingDates(after startDate: Date,
        ...
        if let weeks = combinationComponents.weeksOfYear {
            dates = try dates.flatMap { date, comps in
                try normalizedWeeksOfYear(weeks, anchor: date).map { week in
                    var comps = comps
