    ...
    struct DatesByRecurring: Sendable, Sequence {
        ...
        struct Iterator: Sendable, IteratorProtocol {
            ...
            internal init(
                ...
                baseRecurrence = Calendar.DatesByMatching(
...
extension Calendar.RecurrenceRule {
    ...
    internal func _limitWeekdays(dates: inout [Date], anchor: Date) {
        ...
        let weekdayComponents = self.calendar._weekdayComponents(
            for: weekdays,
            in: parentComponent,
            anchor: anchor)
        dates = dates.filter { date in
...
extension Calendar {
    ...
    func _weekdayComponents(
        for weekdays: [Calendar.RecurrenceRule.Weekday],
    ...
    fileprivate func _normalizedWeeksOfYear(_ weeksOfYear: [Int], anchor: Date) -> [Int] {
        ...
        lazy var lastWeekIdx =
            if daysLeftInLastWeek >= minimumDaysInFirstWeek {
    ...
    fileprivate func _unadjustedDates(
        ...
        if let hours = combinationComponents.hours {
            dates = try dates.flatMap { date, comps in
                ...
                return try hours.map { hour in
                    var comps = comps
                    comps.hour = hour
                    return try dateAfterMatchingHour(
                        startingAt: searchStart, originalStartDate: startDate, components: comps, direction: .forward, findLastMatch: repeatedTimePolicy == .last, isStrictMatching: isStrictMatching, matchingPolicy: matchingPolicy
                    ).map { ($0, comps) } ?? (date, comps)
                }
            }
    ...
    internal func _dates(
        ...
        let results = try unadjustedMatchDates.map { date, components in
            ...
            return (
                try _adjustedDate(
                    date, startingAfter: start, allowStartDate: true, matching: components, adjustedMatchingComponents: adjustedComponents, matchingPolicy: matchingPolicy, repeatedTimePolicy: repeatedTimePolicy, direction: .forward,
                    inSearchingDate: start, previouslyReturnedMatchDate: nil), components
            )
        }
