...
public struct Calendar : Hashable, Equatable, Sendable {
    ...
    ///
    /// - Note: The policy is applied in the order the search encounters the repeated times. When searching with ``Calendar/SearchDirection/backward``, the "first" occurrence is therefore the later of the two in absolute time, and the "last" is the earlier.
