...
private let macRomanMapping: InlineArray<_, UInt8> = [
...
extension String {
    @_lifetime(immortal)
    static func utf8(forMacRomanNonASCII byte: UInt8) -> Span<UInt8> {
        assert(!Unicode.ASCII.isASCII(byte))
        let index = Int(byte - 128) * 3
        var span = macRomanMapping.span.extracting(index ..< index + 3)
        if span[2] == 0 {
...
extension UInt8 {
    init?(macRomanFor scalar: UnicodeScalar) {
        ...
        let utf8 = scalar.utf8

        guard utf8.count <= 3 else {
            return nil
        }

        var mappingSpan = macRomanMapping.span
        var result = 0x80
        let last = utf8.count > 2 ? utf8[2] : 0
        while mappingSpan.count >= 3 {
            if mappingSpan[0] == utf8[0] && mappingSpan[1] == utf8[1] && mappingSpan[2] == last {
                break
            }
            mappingSpan = mappingSpan.extracting(droppingFirst: 3)
            result += 1
        }
        self.init(exactly: result)
    }
}

// MARK: - NextStep

private let nextStepMapping: InlineArray<_, UInt16> = [
...
extension UInt16 {
    init(nextStep codepoint: UInt8) {
        guard !Unicode.ASCII.isASCII(codepoint) else {
            self.init(codepoint)
            return
        }
        let span = nextStepMapping.span

        self = span[Int(codepoint) - 128]
    }
}
