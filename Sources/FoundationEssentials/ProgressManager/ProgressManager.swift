...
@Observable public final class ProgressManager: Sendable {
    ...
    deinit {
        ...
        let (finalSummary, parents) = state.withLock { state in
            ...
            Properties.TotalFileCount.reduce(into: &totalFileCount, value: state.totalFileCount)
            Properties.CompletedFileCount.reduce(into: &completedFileCount, value: state.completedFileCount)
            Properties.TotalByteCount.reduce(into: &totalByteCount, value: state.totalByteCount)
            Properties.CompletedByteCount.reduce(into: &completedByteCount, value: state.completedByteCount)
            Properties.Throughput.reduce(into: &throughput, value: state.throughput)
            Properties.EstimatedTimeRemaining.reduce(into: &estimatedTimeRemaining, value: state.estimatedTimeRemaining)
            
            // MARK: Custom Properties (Int, Int)
            // Aggregate information using self's custom property keys
            for (key, value) in state.customPropertiesInt {
            ...
            // Aggregate information using self's custom property keys
            for (key, value) in state.customPropertiesUInt64 {
                // Set up overall summary
                var summary = key.defaultSummary
            ...
            // MARK: Custom Properties (UInt64, [UInt64])
            // Aggregate information using self's custom property keys
            for (key, value) in state.customPropertiesUInt64Array {
                // Set up overall summary
                var summary = key.defaultSummary
            ...
            // Aggregate information using self's custom property keys
            for (key, value) in state.customPropertiesDouble {
                // Set up overall summary
                var summary = key.defaultSummary
            ...
            // Aggregate information using self's custom property keys
            for (key, value) in state.customPropertiesString {
                // Set up overall summary
                var summary = key.defaultSummary
            ...
            // Aggregate information using self's custom property keys
            for (key, value) in state.customPropertiesURL {
                // Set up overall summary
                var summary = key.defaultSummary
            ...
            // Aggregate information using self's custom property keys
            for (key, value) in state.customPropertiesDuration {
                // Set up overall summary
                var summary = key.defaultSummary
            ...
            for child in state.children {
                ...
                for (key, _) in state.customPropertiesInt {
                ...
                for (key, value) in child.customPropertiesIntSummary {
                    if !state.customPropertiesInt.keys.contains(key) {
                ...
                for (key, _) in state.customPropertiesUInt64 {
                ...
                for (key, value) in child.customPropertiesUInt64Summary {
                    if !state.customPropertiesUInt64.keys.contains(key) {
                        // Set up default summary
                        var summary = key.defaultSummary
                        // Include child's value
                        summary = key.finalSummary(summary, value.value)
                ...
                for (key, _) in state.customPropertiesUInt64Array {
                ...
                for (key, value) in child.customPropertiesUInt64ArraySummary {
                    if !state.customPropertiesUInt64Array.keys.contains(key) {
                        // Set up default summary
                        var summary = key.defaultSummary
                        // Include child's value
                        summary = key.finalSummary(summary, value.value)
                ...
                for (key, _) in state.customPropertiesDouble {
                ...
                for (key, value) in child.customPropertiesDoubleSummary {
                    if !state.customPropertiesDouble.keys.contains(key) {
                        // Set up default summary
                        var summary = key.defaultSummary
                        // Include child's value
                        summary = key.finalSummary(summary, value.value)
                ...
                for (key, _) in state.customPropertiesString {
                ...
                for (key, value) in child.customPropertiesStringSummary {
                    if !state.customPropertiesString.keys.contains(key) {
                        // Set up default summary
                        var summary = key.defaultSummary
                        // Include child's value
                        summary = key.finalSummary(summary, value.value)
                ...
                for (key, _) in state.customPropertiesURL {
                ...
                for (key, value) in child.customPropertiesURLSummary {
                    if !state.customPropertiesURL.keys.contains(key) {
                        // Set up default summary
                        var summary = key.defaultSummary
                        // Include child's value
                        summary = key.finalSummary(summary, value.value)
                ...
                for (key, _) in state.customPropertiesDuration {
                ...
                for (key, value) in child.customPropertiesDurationSummary {
                    if !state.customPropertiesDuration.keys.contains(key) {
                        // Set up default summary
                        var summary = key.defaultSummary
                        // Include child's value
                        summary = key.finalSummary(summary, value.value)
